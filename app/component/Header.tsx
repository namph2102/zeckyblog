...
const Header: React.FC<HeaderProps> = ({ listMenu }) => {
  ...
  return (
    ...
    <header className="sticky top-0 left-0 right-0 ease-in duration-300 bg-main py-2 z-[4]">
  ...
  <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl md:p-4 p-2">
   ...
   <sub className="text-white text-base font-medium">ecky</sub>
    ...
    <div id="mega-menu-full" className="items-center justify-between md:static absolute left-0 right-0 top-16 font-medium hidden w-full md:flex md:w-auto md:order-1">
      <ul className="flex flex-col  rounded-lg md:flex-row md:space-x-8 md:mt-0 md:bg-transparent bg-primary ">
        <li>
          <Link href="/" className="block cursor-pointer" aria-current="page">Trang chủ</Link>
        </li>
        <li>
          <label htmlFor="idcheckbox" data-collapse-toggle="mega-menu-full-dropdown" className="flex items-center w-full  text-white hover:text-hover cursor-pointer">Danh mục <svg className="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
              <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="m1 1 4 4 4-4" />
            </svg></label>
        </li>
        <li>
          <Link href="/tin-tuc" className="cursor-pointer block">Tin tức</Link>
        </li>
        <li>
          <Link href="/hoc-lap-trinh" className="cursor-pointer block">Học lập trình</Link>
        </li>
  ...
  <div id="mega-menu-full-dropdown" className=" border-gray-200 shadow-sm bg-primary absolute md:top-full top-0 left-0 right-0">
